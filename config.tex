...
%%% Математика (загружается ДО шрифтов, чтобы избежать конфликтов)
\usepackage{amsmath}
\usepackage{mathtools}

%%% Шрифты
\usepackage{tempora}  % Times-подобный шрифт с поддержкой кириллицы
\let\C\relax        % resolves conflict: babel/kernel defines \C before newtxmath
\usepackage[bigdelims,vvarbb]{newtxmath}
\let\openbox\relax  % resolves conflict between newtxmath and amsthm
\usepackage{amsthm}
...
%%% amssymb — newtxmath уже предоставляет символы, amssymb не нужен
%% \usepackage{amssymb}
